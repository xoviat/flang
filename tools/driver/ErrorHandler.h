...
//===----------------------------------------------------------------------===//
//
// We designed flang's error handlers with the following goals in mind:
//
...
//    number of functions in flang with ErrorOr. With that approach, if some
...
//
// The current flang's error handling mechanism is simple:
//
...
// if you are using flang as a subroutine in other program, and with that you
// can find only one error at a time.
//
// warn() doesn't do anything but printing out a given message.
//
// It is not recommended to use llvm::outs() or llvm::errs() directly in flang
// because they are not thread-safe. The functions declared in this file are
// thread-safe.
//
//===----------------------------------------------------------------------===//

#ifndef FLANG_ERRORHANDLER_H
#define FLANG_ERRORHANDLER_H
...
LLVM_ATTRIBUTE_NORETURN void exitflang(int Val);
